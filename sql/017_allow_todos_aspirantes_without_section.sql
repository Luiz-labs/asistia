-- =====================================================================
-- SCRIPT DE BASE DE DATOS: PERMITIR TODOS_ASPIRANTES EN ASPIRANTES SIN SECCIÓN
...
BEGIN
  ...
  -- VALIDACIÓN DE SECCIÓN CON EXCEPCIÓN PARA TODOS_ASPIRANTES
  IF COALESCE(v_seccion, '') = '' OR v_seccion IN ('GENERAL', 'DOMINICAL') THEN
    -- Verificar si existe una programación especial global hoy que permita la asistencia
    SELECT 
  ...
  -- RESOLUCIÓN DE REGLAS ESTÁNDAR (Si el alumno sí tiene sección válida y no se ha resuelto por el fallback global de sección ausente)
  ...
  -- FALLBACK DE CALENDARIO: Si el alumno tiene sección pero no tiene jornada regular hoy, verificar si existe programación global
  IF NOT v_tiene_contexto THEN
    SELECT 
        'CALENDARIO_GLOBAL', -- jornada_codigo
        'Sesión especial de calendario', -- jornada_label
